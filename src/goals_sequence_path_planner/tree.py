...
class Tree:
	def __init__(self,
		...
		# TODO Improve This
		self.goal_pose = start_pose_other_tree
		self.grid = grid
		...
		if self.virtual_obstacles:
			self.start_virtual_obstacle = VirtualObstacle(self.start_pose,
											         	  virtual_obs_tolerance,
											         	  virtual_obs_radius,
											         	  pose_status_goal=start_virtual_obs)

			self.goal_virtual_obstacle = VirtualObstacle(self.goal_pose,
											             virtual_obs_tolerance,
											             virtual_obs_radius,
	...
	def choose_parent(self, new_node, parent):
		...
		for node in self.nodes:
			...
			d = distance(node.point, new_node.point)

			# connection cost node to new_node
			cost_n_2_new_n = node.cost + distance(node.point, new_node.point)

			# connection cost node to parent
			cost_p_2_new_n = parent.cost + distance(parent.point, new_node.point)

			if d < self.optimization_radius and cost_n_2_new_n < cost_p_2_new_n:
				if self.obstacle_free(node.point, new_node.point):
					parent = node

		new_node.cost = parent.cost + distance(parent.point, new_node.point)
	...
	def rewire(self, new_node):
		...
		for node in self.nodes:
			...
			# connection cost node to new_node
			cost_new_n_2_n = new_node.cost + distance(node.point, new_node.point)

			if node != new_n_p and d < self.optimization_radius and cost_new_n_2_n < node.cost:
				if self.obstacle_free(node.point, new_node.point):
					# Now the node parent is the new node
					node.parent = new_node
					node.cost = new_node.cost + distance(node.point, new_node.point)				
	...
	def steer(self, p1, p2):
		...
		if dist < self.epsilon:
			...
			return p1[0] + self.epsilon*math.cos(theta), p1[1] + self.epsilon*math.sin(theta)

	def get_nearest(self, p_rand):
		""" Returns the nearest node of the list."""
		n_nearest = self.nodes[0]
		for node in self.nodes:
			if distance(node.point, p_rand) < distance(n_nearest.point, p_rand):
				n_nearest = node
	...
	def attempt_connect(self, external_node):
		...
		if distance(n_nearest.point, external_node.point) < self.goal_tolerance:
			# check collision
	...
	def add_nodes_to_tree(self, external_nodes, parent_node):
		...
		for node in external_nodes:
			...
			node.cost = current_parent.cost + distance(node.point, current_parent.point)
	...
	def path_optimization(self):
		...
		while current_node.parent.parent != None:
			if self.obstacle_free(current_node.point, current_node.parent.parent.point):
				...
				dist_cost = distance(current_node.parent.point, current_node.point)
	...
	def obstacle_free(self, p1, p2):
		...
		dist = distance(p1, p2)
		n = 1
		if dist < self.obs_resolution:
			if self.collision(p2):
				return False
			else:
				return True
		else:
			for i in range(int(math.floor(dist/self.obs_resolution))):
